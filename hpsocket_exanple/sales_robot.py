...
class Server(HP_TcpPackServer):
    ...
    def OnReceive(self, Sender, ConnID, Data):
        ...
        if statu == 1:
            ...
            if ret.get('type') == 48:
                msg = ret.get('msg')
                label = re.search('label="(.*?)"', msg)
                print(label)
                content['label'] = label

                statu += 1
            else:
                data = send_msg(ret.get('sender'), '你发送的不是位置信息，请重新发送！')
                self.Send(Sender=Sender, ConnID=ConnID, Data=data)

        if statu == 2:
            # 处理拍照
            if ret.get('type') != 3:
                data = send_msg(ret.get('sender'), '你发送的不是图片，请重新发送！')
            self.Send(Sender=Sender, ConnID=ConnID, Data=data)
            ...
            if not wait_file(dst_path):
                ...
                return
            content['img_path'] = dst_path
            statu += 1

        if ret.get('type') != 1:
            data = send_msg(ret.get('sender'), '输入信息应为文本格式，请重新发送！：')
            self.Send(Sender=Sender, ConnID=ConnID, Data=data)

        if statu == 3:
            data = send_msg(ret.get('sender'), '请输入要拜访的公司名称：')
            self.Send(Sender=Sender, ConnID=ConnID, Data=data)
            statu += 1

        if statu == 4:
            # 接收保存的公司名称
            company_name = ret.get('msg')
            print(company_name)
            content['company_name'] = company_name
            statu += 1

        if statu == 5:
            text = '您是第一次拜访客户，请选择输入项：\n' \
                   '1.联系人\n' \
                   '2.联系方式\n' \
                   '3.项目摘要\n' \
                   '4.最新状态\n' \
        ...
        if statu == 6:
            if ret.get('msg').strip() == 1:
                data = send_msg(ret.get('sender'), '请输入联系人：')
                self.Send(Sender=Sender, ConnID=ConnID, Data=data)
                n = 1
            if n == 1:
                # 保存联系人
                contact_person = ret.get('msg')
                print(contact_person)
                content['contact_person'] = contact_person

            if ret.get('msg').strip() == 2:
                data = send_msg(ret.get('sender'), '请输入联系方式：')
                self.Send(Sender=Sender, ConnID=ConnID, Data=data)
                n = 2
            if n == 2:
                # 保存联系方式
                contact_details = ret.get('msg')
                print(contact_details)
                content['contact_details'] = contact_details


            if ret.get('msg').strip() == 3:
                data = send_msg(ret.get('sender'), '请输入项目摘要：')
                self.Send(Sender=Sender, ConnID=ConnID, Data=data)
                n = 3
            if n == 3:
                # 保存项目摘要
                project_abstract = ret.get('msg')
                print(project_abstract)
                content['project_abstract'] = project_abstract


            if ret.get('msg').strip() == 4:
                data = send_msg(ret.get('sender'), '请输入最新状态：')
                self.Send(Sender=Sender, ConnID=ConnID, Data=data)
                n = 4
            if n == 4:
                # 保存最新状态
                latest_status = ret.get('msg')
                print(latest_status)
                content['latest_status'] = latest_status

            if ret.get('msg').strip() == 5:
                data = send_msg(ret.get('sender'), '请重新输入：')
                self.Send(Sender=Sender, ConnID=ConnID, Data=data)
                n = 5
            if n == 5:
                statu -= 1

            if ret.get('msg').strip() == 6:
                n = 6
            if n == 6:
                # 退出
                statu += 1
        ...
        if statu == 7:
            ...
            text = '位置：{label}，' \
                   ...
                   '公司名称：{company_name}，' \
                   '联系人：{contact_person}，' \
                   '联系方式：{contact_details}，' \
                   '项目摘要：{project_abstract}，' \
                   '最新状态：{latest_status}'.format(**content)
            data = send_msg(ret.get('sender'), text)
            self.Send(Sender=Sender, ConnID=ConnID, Data=data)
        ...
        #     if ret.get('type') == 1 and '时间' in ret.get('msg'):
